...
class AdminRouter:
    ...
    def _register_routes(self) -> None:
        """注册路由方法。"""
        # 学生管理（普通管理员和超级管理员可用）
        self._router.get("/students", response_model=UserListResponse)(
            self.get_students
        )
        self._router.post("/students", response_model=UserResponse)(self.create_student)
        self._router.patch(
            "/students/{user_id}/status", response_model=OperationResponse
        )(self.set_student_status)
        self._router.delete("/students/{user_id}", response_model=OperationResponse)(
            self.delete_student
        )
        ...
        # 系统操作
        self._router.post("/redetect", response_model=OperationResponse)(
            self.redetect_all_emails
        )
    ...
    async def get_students(
        ...
        """获取学生用户列表。"""
        self._logger.info(f"获取学生列表: page={page}")
        users, total = await self._admin_service.get_students(
    ...
    async def create_student(
        ...
        """创建学生用户。"""
        self._logger.info(f"创建学生: student_id={request.student_id}")
        user = await self._admin_service.create_user(request, role="user")
        if not user:
    ...
    async def set_student_status(
        ...
        if success:
            return OperationResponse(success=True, message="学生状态已更新")
    ...
    async def delete_student(
        ...
        if success:
            return OperationResponse(success=True, message="学生已删除")
        raise HTTPException(status_code=404, detail="用户不存在")
    ...
    async def redetect_all_emails(
        ...
        通常在更新白名单规则或检测算法后使用。
        """
        self._logger.info(f"用户 {current_user.student_id} 触发全局邮件重新检测")
        result = await self._admin_service.redetect_all_emails()
        return OperationResponse(success=True, message=result["message"])
    ...
    async def get_system_settings(
        ...
        """获取系统设置。"""
        return await self._admin_service.get_system_settings()
    ...
    async def update_system_settings(
        ...
        """更新系统设置。"""
        return await self._admin_service.update_system_settings(request)
